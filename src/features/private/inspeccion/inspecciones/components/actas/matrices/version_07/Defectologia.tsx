...
export const Defectologia = ({ inspeccion }: Props) => {
  ...
  const evaluarItem = (
    ...
    if (val === true || val === "SI" || val === "si" || val === "true") {
      return {
        critico: false,
        noCritico: false,
        cumple: true,
    ...
    if (val === false || val === "NO" || val === "no" || val === "false") {
      if (resultadoGlobal === "defectos_criticos") {
        return { critico: true, noCritico: false, cumple: false, na: false };
      }
      if (resultadoGlobal === "defectos_no_criticos") {
        return { critico: false, noCritico: true, cumple: false, na: false };
      }
      return {
  ...
  const hermEval = evaluarItem(
    cv?.hermeticidad?.cumple,
    "critico",
    cv?.hermeticidad?.na,
  );
  const trazadoEval = evaluarItem(
    cv?.trazado_general,
    "no_critico",
    (cv as any)?.trazado_general_na,
  );
  const materialesEval = evaluarItem(
    cv?.materiales,
    "critico",
    (cv as any)?.materiales_na,
  );
  const getValvularVal = (obj: any) => {
    if (!obj) return undefined;
    if (obj.valvular !== undefined) return obj.valvular;
    if (obj.valvulas !== undefined) return obj.valvulas;
    if (obj.existencia_operatividad_valvular !== undefined)
      return obj.existencia_operatividad_valvular;
    if (obj.existencia_operatividad_valvulas !== undefined)
      return obj.existencia_operatividad_valvulas;
    return undefined;
  };
  ...
  const valvularEval = evaluarItem(
    getValvularVal(cv),
    "critico",
    (cv as any)?.valvular_na ?? (cv as any)?.valvulas_na,
  );

  return (
    <div className="w-full border-l border-r border-b border-black font-arial box-border flex flex-col bg-white text-black">
      {/* Fila Encabezado: 6. DEFECTOLOGIA | Critico | No critico | Cumple | N/A */}
      <div className="flex flex-row w-full border-b border-black bg-[#f2f2f2] box-border min-h-[16px]">
        <div className="w-[68%] border-r border-black text-[7.5pt] font-bold text-center p-[2px_4px] box-border uppercase flex items-center justify-center">
          6. DEFECTOLOGIA
        </div>
        <div className="w-[8%] border-r border-black text-[7.2pt] text-center p-[2px_2px] box-border flex items-center justify-center font-bold">
          Critico
        </div>
        <div className="w-[8%] border-r border-black text-[7.2pt] text-center p-[2px_2px] box-border flex items-center justify-center font-bold">
          No critico
        ...
        <div className="w-[8%] text-[7.2pt] text-center p-[2px_2px] box-border flex items-center justify-center font-bold">
          N/A
        </div>
      </div>
      ...
      <div className="flex flex-row w-full border-b border-black box-border min-h-[17px] text-[6.8pt]">
        <div className="w-[68%] border-r border-black px-[6px] py-[2px] flex items-center justify-between box-border">
          <span className="whitespace-nowrap">Hermeticidad de la linea matriz</span>
          <div className="flex items-center gap-[8px] mr-[6px]">
        ...
        <div className="w-[8%] border-r border-black flex items-center justify-center box-border">
        ...
        <div className="w-[8%] flex items-center justify-center box-border">
          <CheckBox checked={hermEval.na} />
        </div>
      </div>

      {/* Fila 2: Trazado general de instalación */}
      <div className="flex flex-row w-full border-b border-black box-border min-h-[17px] text-[6.8pt]">
        <div className="w-[68%] border-r border-black px-[6px] py-[2px] flex items-center box-border">
        ...
        <div className="w-[8%] flex items-center justify-center box-border">
          <CheckBox checked={trazadoEval.na} />
        </div>
      </div>
      ...
      <div className="flex flex-row w-full border-b border-black box-border min-h-[17px] text-[6.8pt]">
        ...
        <div className="w-[8%] flex items-center justify-center box-border">
          <CheckBox checked={materialesEval.na} />
        </div>
      </div>
      ...
      <div className="flex flex-row w-full box-border min-h-[17px] text-[6.8pt]">
        ...
        <div className="w-[8%] flex items-center justify-center box-border">
          <CheckBox checked={valvularEval.na} />
        </div>
      </div>
    </div>
  );
